...
function PanelCompras({ S, comprasData, setComprasData, personal, esJefa, data={}, updateZona=()=>{}, MACROZONAS_BASE=[] }) {
  ...
  // Bodega Macrozona 39
  const BODEGA_ID = 39;
  const bodegaData = data[BODEGA_ID] || {};
  const materialesBodega = bodegaData.materiales || [];

  const ingresarAStock = (compra) => {
    const nombre = compra.descripcion;
    const cant   = Number(compra.cantidad)||1;
    const unidad = compra.unidad||"unidad";
    const existing = materialesBodega.find(m=>m.nombre.toLowerCase()===nombre.toLowerCase());
    const hoyStr = new Date().toISOString().slice(0,10);
    let nuevos;
    if(existing) {
      nuevos = materialesBodega.map(m=>m.nombre.toLowerCase()===nombre.toLowerCase()?{
        ...m, stockActual:(m.stockActual||0)+cant,
        movimientos:[{id:Date.now(),tipo:"entrada",cantidad:cant,motivo:`Compra ${compra.tipoDoc} ${compra.nDocumento||""} — ${compra.proveedor||""}`,responsable:compra.responsable||"",fecha:hoyStr},...(m.movimientos||[])].slice(0,50),
      }:m);
    } else {
      nuevos = [...materialesBodega,{
        id:Date.now(), nombre, unidad, stockActual:cant, stockMinimo:0, obs:`Ingresado desde compra ${compra.tipoDoc||""} ${compra.nDocumento||""}`,
        movimientos:[{id:Date.now()+1,tipo:"entrada",cantidad:cant,motivo:`Compra inicial — ${compra.proveedor||""}`,responsable:compra.responsable||"",fecha:hoyStr}],
      }];
    }
    updateZona(BODEGA_ID, {materiales:nuevos});
  };

  const hoy = new Date();
  const [subTab, setSubTab] = React.useState("fondo");
  const [showForm, setShowForm] = React.useState(false);
  const [showCuentaForm, setShowCuentaForm] = React.useState(false);
  const [showRendForm, setShowRendForm] = React.useState(false);
  const [showReembolsoForm, setShowReembolsoForm] = React.useState(false);
  const [nuevaCuenta, setNuevaCuenta] = React.useState("");
  const [editId, setEditId] = React.useState(null);
  const [filtroCuenta, setFiltroCuenta] = React.useState("todas");
  const [filtroMes, setFiltroMes] = React.useState("todos");
  ...
  const emptyForm = {
    fecha: hoy.toISOString().slice(0,10),
    proveedor:"", rut:"", nDocumento:"", tipoDoc:"Factura",
    descripcion:"", categoria:"", cuenta:"",
    cantidad:1, unidad:"unidad", precioUnitario:"", totalNeto:"", iva:"", totalBruto:"",
    fechaPago:"", bancoPago:"", nTransferencia:"",
    responsable:"", estado:"pendiente", obs:"", ingresoStock:true,
  };
  const [form, setForm] = React.useState(emptyForm);
  const labelSt = {fontSize:10,color:"#6aaa7a",letterSpacing:"0.6px",display:"block",marginBottom:3,textTransform:"uppercase"};

  // ── Cálculos fondo ────────────────────────────────────────────────────────
  const gastadoPendiente = compras.filter(c=>c.estado==="pendiente"||c.estado==="pagada")
    .reduce((a,c)=>a+Number(c.totalBruto||c.totalNeto||0),0);
  const totalReembolsado = rendiciones.reduce((a,r)=>a+(r.reembolso?Number(r.montoReembolso||0):0),0);
  const saldoDisponible  = fondo - gastadoPendiente;
  const pctUsado = fondo ? Math.round((gastadoPendiente/fondo)*100) : 0;
  const colorSaldo = saldoDisponible > fondo*0.4 ? "#22c55e" : saldoDisponible > fondo*0.15 ? "#f59e0b" : "#ef4444";

  // ── Cálculos automáticos formulario ──────────────────────────────────────
  const calcTotales = (f) => {
    const cant=Number(f.cantidad)||1, pu=Number(f.precioUnitario)||0;
    const neto=Math.round(cant*pu), iva=Math.round(neto*0.19);
    return {totalNeto:neto||"", iva:iva||"", totalBruto:(neto+iva)||""};
  };
  const updateForm = (patch) => {
    setForm(p=>{
      const next={...p,...patch};
      if(patch.cantidad!==undefined||patch.precioUnitario!==undefined) return {...next,...calcTotales(next)};
      return next;
    });
  };
  ...
  // ── CRUD compras ──────────────────────────────────────────────────────────
  const guardar = () => {
    if(!form.descripcion.trim()||!form.cuenta) return;
    const nueva = {...form, id: editId||Date.now()};
    if(editId) { set({compras:compras.map(c=>c.id===editId?nueva:c)}); setEditId(null); }
    else        { set({compras:[nueva,...compras]}); if(form.ingresoStock) ingresarAStock(nueva); }
    setForm(emptyForm); setShowForm(false);
  };
  const eliminar = (id) => set({compras:compras.filter(c=>c.id!==id)});
  const editar   = (c)  => {setForm({...c});setEditId(c.id);setShowForm(true);setSubTab("lista");};
  const marcarPagada = (id) => set({compras:compras.map(c=>c.id===id?{...c,estado:"pagada",fechaPago:c.fechaPago||hoy.toISOString().slice(0,10)}:c)});

  // ── Rendición ─────────────────────────────────────────────────────────────
  const crearRendicion = () => {
    if(!seleccionadas.length) return;
    const items = compras.filter(c=>seleccionadas.includes(c.id));
    const total = items.reduce((a,c)=>a+Number(c.totalBruto||c.totalNeto||0),0);
    const nueva = {
      id:Date.now(), fecha:rendForm.fecha, obs:rendForm.obs,
      items:seleccionadas, total, estado:"presentada", reembolso:false,
      montoReembolso:0, fechaReembolso:"", nTransReembolso:"",
    };
    set({
      compras: compras.map(c=>seleccionadas.includes(c.id)?{...c,estado:"en_rendicion"}:c),
      rendiciones: [nueva,...rendiciones],
    });
    setSeleccionadas([]); setRendForm({fecha:hoy.toISOString().slice(0,10),obs:""}); setShowRendForm(false);
  };

  // ── Reembolso del Estadio ─────────────────────────────────────────────────
  const registrarReembolso = (rendId) => {
    if(!reembolsoForm.monto) return;
    set({
      rendiciones: rendiciones.map(r=>r.id===rendId?{...r,
        reembolso:true, estado:"reembolsada",
        montoReembolso:Number(reembolsoForm.monto),
        fechaReembolso:reembolsoForm.fecha,
        nTransReembolso:reembolsoForm.nTransferencia,
        bancoReembolso:reembolsoForm.banco,
        obsReembolso:reembolsoForm.obs,
      }:r),
      compras: compras.map(c=>
        rendiciones.find(r=>r.id===rendId)?.items?.includes(c.id)?{...c,estado:"rendida"}:c
      ),
    });
    setReembolsoForm({fecha:hoy.toISOString().slice(0,10),monto:"",banco:"",nTransferencia:"",obs:""});
    setShowReembolsoForm(false);
  ...
  // ── Resumen por cuenta ────────────────────────────────────────────────────
  const totalGeneral = compras.reduce((a,c)=>a+Number(c.totalBruto||c.totalNeto||0),0);
  const porCuenta = cuentas.map(cu=>{
    const items=compras.filter(c=>c.cuenta===cu);
    const total=items.reduce((a,c)=>a+Number(c.totalBruto||c.totalNeto||0),0);
    return {cuenta:cu,total,n:items.length};
  }).filter(x=>x.total>0).sort((a,b)=>b.total-a.total);

  const mesesDisp=[];
  for(let i=5;i>=0;i--){
    const d=new Date(hoy.getFullYear(),hoy.getMonth()-i,1);
    mesesDisp.push({key:`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,"0")}`,label:`${MESES_COMPRAS[d.getMonth()]} ${d.getFullYear()}`});
  }
  const porMes=mesesDisp.map(m=>{
    const total=compras.filter(c=>(c.fecha||"").startsWith(m.key)).reduce((a,c)=>a+Number(c.totalBruto||c.totalNeto||0),0);
    return {...m,total};
  });
  const maxMes=Math.max(...porMes.map(m=>m.total),1);
  const mesesUnicos=[...new Set(compras.map(c=>(c.fecha||"").slice(0,7)).filter(Boolean))].sort((a,b)=>b.localeCompare(a));
  ...
  const ESTADO_C = {
    pendiente:   {color:"#f59e0b",bg:"rgba(245,158,11,0.1)",  label:"⏳ Pendiente pago"},
    pagada:      {color:"#3b82f6",bg:"rgba(59,130,246,0.1)",  label:"💳 Pagada"},
    en_rendicion:{color:"#a78bfa",bg:"rgba(167,139,250,0.1)", label:"📤 En rendición"},
    rendida:     {color:"#22c55e",bg:"rgba(34,197,94,0.1)",   label:"✅ Rendida"},
    rechazada:   {color:"#ef4444",bg:"rgba(239,68,68,0.1)",   label:"❌ Rechazada"},
  };
  ...
  return (
    <div className="ein">
      <div style={{marginBottom:16}}>
        <h1 style={{fontFamily:"'Playfair Display',serif",fontSize:24,fontWeight:900,marginBottom:4}}>🛒 Compras y Rendición</h1>
        <p style={{color:"#6aaa7a",fontSize:14}}>Fondo fijo rotativo · Áreas Verdes Estadio Español</p>
      </div>

      {/* ── PANEL FONDO — siempre visible ── */}
      <div style={{...S.card,padding:18,marginBottom:18,borderColor:colorSaldo+"40"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"start",flexWrap:"wrap",gap:12,marginBottom:14}}>
          <div>
            <div style={{fontSize:11,color:"#6aaa7a",letterSpacing:"0.6px",marginBottom:4,textTransform:"uppercase"}}>💼 Fondo disponible</div>
            <div style={{fontFamily:"'Playfair Display',serif",fontSize:32,fontWeight:900,color:colorSaldo}}>
        ...
        {/* Barra de fondo */}
        <div style={{background:"rgba(255,255,255,0.07)",borderRadius:6,height:10,overflow:"hidden",marginBottom:6}}>
          <div style={{width:`${Math.min(pctUsado,100)}%`,height:"100%",background:colorSaldo,borderRadius:6,transition:"width .4s"}}/>
        </div>
        <div style={{display:"flex",justifyContent:"space-between",fontSize:11,color:"#5a8a6a"}}>
          <span>{pctUsado}% comprometido</span>
          {esJefa&&<button style={{...S.btn,fontSize:10,padding:"2px 8px",background:"rgba(255,255,255,0.06)",color:"#7aaa80",border:"1px solid rgba(255,255,255,0.1)"}}
      ...
      {/* Sub-tabs */}
      <div style={{display:"flex",gap:6,marginBottom:18,flexWrap:"wrap"}}>
        {[["fondo","📊 Resumen"],["lista","📋 Compras"],["rendiciones","📤 Rendiciones"],["cuentas","🏷️ Cuentas"]].map(([t,l])=>{
          const badge = t==="lista"?compras.filter(c=>c.estado==="pendiente").length:t==="rendiciones"?rendiciones.filter(r=>!r.reembolso).length:0;
          return (
            <button key={t} className={`tab${subTab===t?" on":""}`} onClick={()=>{setSubTab(t);setShowForm(false);}} style={{position:"relative"}}>
              {l}
              {badge>0&&<span style={{position:"absolute",top:1,right:1,background:"#ef4444",color:"#fff",borderRadius:"50%",width:14,height:14,fontSize:9,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700}}>{badge}</span>}
            </button>
          );
        })}
      </div>
        ...
        <div className="ein">
          {compras.length===0?(
            <div style={{...S.card,padding:40,textAlign:"center",color:"#4a8a5a"}}>
              <div style={{fontSize:36,marginBottom:10}}>🛒</div>
              <div style={{fontFamily:"'Playfair Display',serif",fontSize:16}}>Sin compras registradas</div>
              <div style={{fontSize:13,marginTop:6}}>Ve a 📋 Compras para agregar</div>
            </div>
          ):(
            <>
              {/* Por cuenta */}
              ...
              <div style={{...S.card,padding:0,overflow:"hidden",marginBottom:20}}>
                {porCuenta.map((c,i)=>{
                  const pct=totalGeneral?Math.round((c.total/totalGeneral)*100):0;
                  return (
                    <div key={c.cuenta} style={{padding:"11px 16px",borderBottom:"1px solid rgba(255,255,255,0.06)",background:i%2===0?"transparent":"rgba(255,255,255,0.018)",cursor:"pointer"}}
                      onClick={()=>{setFiltroCuenta(c.cuenta);setSubTab("lista");}}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
                        <span style={{fontSize:13,fontWeight:600}}>{c.cuenta}</span>
                        <div style={{display:"flex",gap:10,alignItems:"center"}}>
                          <span style={{fontSize:11,color:"#6aaa7a"}}>{c.n} compra{c.n!==1?"s":""}</span>
                ...
                <div style={{padding:"11px 16px",display:"flex",justifyContent:"space-between",background:"rgba(59,130,246,0.08)",borderTop:"1px solid rgba(59,130,246,0.2)"}}>
                  <span style={{fontWeight:700,fontSize:13}}>TOTAL GENERAL</span>
                  <span style={{fontFamily:"'Playfair Display',serif",fontSize:15,fontWeight:700,color:"#93c5fd"}}>${totalGeneral.toLocaleString("es-CL")}</span>
                </div>
              </div>
              ...
              <div style={{...S.card,padding:18,marginBottom:20}}>
                <div style={{display:"flex",alignItems:"flex-end",gap:6,height:100,justifyContent:"space-around"}}>
                  {porMes.map(m=>{
                    ...
                    const esActual=m.key===`${hoy.getFullYear()}-${String(hoy.getMonth()+1).padStart(2,"0")}`;
                    return (
                      <div key={m.key} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,flex:1}}>
                        {m.total>0&&<div style={{fontSize:9,color:"#93c5fd",textAlign:"center"}}>${Math.round(m.total/1000)}k</div>}
                        <div style={{width:"100%",background:esActual?"rgba(59,130,246,0.4)":"rgba(59,130,246,0.15)",borderRadius:"3px 3px 0 0",height:`${Math.max(h,m.total>0?4:0)}%`,border:esActual?"1px solid rgba(59,130,246,0.5)":"none"}}/>
                        <div style={{fontSize:9,color:esActual?"#93c5fd":"#5a8a6a",fontWeight:esActual?700:400,textAlign:"center"}}>{m.label.split(" ")[0]}</div>
                      </div>
...
Si un campo no está visible o no aplica, usa cadena vacía "" o 0 para números.`}
            ...
            {seleccionadas.length>0&&(
              <button style={{...S.btn,background:"rgba(167,139,250,0.2)",color:"#c4b5fd",border:"1px solid rgba(167,139,250,0.35)",fontSize:12}}
                ...
                📤 Rendir seleccionadas ({seleccionadas.length})
              </button>
            )}
            <select style={{...S.input,flex:1,minWidth:120,fontSize:12}} value={filtroCuenta} onChange={e=>setFiltroCuenta(e.target.value)}>
              <option value="todas">Todas las cuentas</option>
          ...
          {showRendForm&&(
            <div style={{...S.card,padding:16,marginBottom:14,background:"rgba(167,139,250,0.06)",borderColor:"rgba(167,139,250,0.25)"}}>
              <div style={{fontFamily:"'Playfair Display',serif",fontSize:14,marginBottom:12,color:"#c4b5fd"}}>
                📤 Crear rendición — {seleccionadas.length} compra{seleccionadas.length!==1?"s":" "} · ${compras.filter(c=>seleccionadas.includes(c.id)).reduce((a,c)=>a+Number(c.totalBruto||c.totalNeto||0),0).toLocaleString("es-CL")}
              </div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
                <div><label style={labelSt}>Fecha presentación</label><input type="date" style={S.input} value={rendForm.fecha} onChange={e=>setRendForm(p=>({...p,fecha:e.target.value}))}/></div>
                <div><label style={labelSt}>Observaciones</label><input style={S.input} placeholder="Ej: Rendición junio 2026" value={rendForm.obs} onChange={e=>setRendForm(p=>({...p,obs:e.target.value}))}/></div>
          ...
          {showForm&&(
            <div style={{...S.card,padding:20,marginBottom:16}} className="ein">
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:12}}>
                ...
                <div><label style={labelSt}>Tipo documento</label>
                  <select style={S.input} value={form.tipoDoc} onChange={e=>updateForm({tipoDoc:e.target.value})}>
                    {["Factura","Boleta","Nota de Pedido","Cotización","Orden de Compra","Otro"].map(t=><option key={t}>{t}</option>)}
                  </select>
                </div>
                <div><label style={labelSt}>Proveedor</label><input style={S.input} placeholder="Nombre empresa / persona" value={form.proveedor} onChange={e=>updateForm({proveedor:e.target.value})}/></div>
                <div><label style={labelSt}>RUT proveedor</label><input style={S.input} placeholder="12.345.678-9" value={form.rut} onChange={e=>updateForm({rut:e.target.value})}/></div>
                <div><label style={labelSt}>N° Documento</label><input style={S.input} placeholder="ej: 7609" value={form.nDocumento} onChange={e=>updateForm({nDocumento:e.target.value})}/></div>
                <div><label style={labelSt}>Responsable compra</label>
                  <select style={S.input} value={form.responsable} onChange={e=>updateForm({responsable:e.target.value})}>
                    <option value="">Seleccionar...</option>
                    {listaPersonal.map(p=><option key={p.id} value={p.nombre}>{p.nombre}</option>)}
                    <option value="Carmen Luz Hermosilla">Carmen Luz Hermosilla (Jefa)</option>
                  </select>
                </div>
                <div style={{gridColumn:"1/-1"}}><label style={labelSt}>Descripción</label><input style={S.input} placeholder="ej: Amistar TOP 1L × 2 unidades" value={form.descripcion} onChange={e=>updateForm({descripcion:e.target.value})}/></div>
                <div><label style={labelSt}>Categoría</label><input style={S.input} placeholder="ej: Fungicida, Herramienta..." value={form.categoria} onChange={e=>updateForm({categoria:e.target.value})}/></div>
                <div><label style={labelSt}>Cuenta a imputar</label>
                  <select style={S.input} value={form.cuenta} onChange={e=>updateForm({cuenta:e.target.value})}>
                    <option value="">Seleccionar cuenta...</option>
                    {cuentas.map(c=><option key={c}>{c}</option>)}
                  </select>
                </div>
                <div style={{display:"flex",gap:6}}>
                  <div style={{flex:2}}><label style={labelSt}>Cantidad</label><input type="number" min={1} step={0.01} style={S.input} value={form.cantidad} onChange={e=>updateForm({cantidad:e.target.value})}/></div>
                  <div style={{flex:2}}><label style={labelSt}>Unidad</label>
                    <select style={S.input} value={form.unidad} onChange={e=>updateForm({unidad:e.target.value})}>
                      {["unidad","kg","L","ml","g","m²","m","hora","servicio"].map(u=><option key={u}>{u}</option>)}
                    </select>
                  </div>
                </div>
                <div><label style={labelSt}>Precio unitario neto ($)</label><input type="number" min={0} style={S.input} placeholder="sin IVA" value={form.precioUnitario} onChange={e=>updateForm({precioUnitario:e.target.value})}/></div>

                {/* Totales */}
                <div style={{gridColumn:"1/-1",background:"rgba(59,130,246,0.06)",border:"1px solid rgba(59,130,246,0.2)",borderRadius:10,padding:"12px 14px"}}>
                  <div style={{fontSize:11,color:"#93c5fd",marginBottom:8,textTransform:"uppercase",letterSpacing:"0.5px"}}>💰 Totales</div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10}}>
                    <div><label style={labelSt}>Neto ($)</label><input type="number" style={{...S.input,background:"rgba(59,130,246,0.08)"}} value={form.totalNeto} onChange={e=>updateForm({totalNeto:e.target.value})}/></div>
                    <div><label style={labelSt}>IVA 19% ($)</label><input type="number" style={{...S.input,background:"rgba(59,130,246,0.08)"}} value={form.iva} onChange={e=>updateForm({iva:e.target.value})}/></div>
                    <div><label style={labelSt}>Total bruto ($)</label><input type="number" style={{...S.input,background:"rgba(59,130,246,0.12)",fontWeight:700}} value={form.totalBruto} onChange={e=>updateForm({totalBruto:e.target.value})}/></div>
                  </div>
                </div>

                {/* Pago por transferencia */}
                <div style={{gridColumn:"1/-1",background:"rgba(34,197,94,0.05)",border:"1px solid rgba(34,197,94,0.2)",borderRadius:10,padding:"12px 14px"}}>
                  <div style={{fontSize:11,color:"#86efac",marginBottom:8,textTransform:"uppercase",letterSpacing:"0.5px"}}>💳 Pago por transferencia</div>
                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10}}>
                    <div><label style={labelSt}>Fecha pago</label><input type="date" style={S.input} value={form.fechaPago} onChange={e=>updateForm({fechaPago:e.target.value})}/></div>
                    <div><label style={labelSt}>Banco origen</label><input style={S.input} placeholder="ej: BancoEstado" value={form.bancoPago} onChange={e=>updateForm({bancoPago:e.target.value})}/></div>
                    <div><label style={labelSt}>N° Transferencia</label><input style={S.input} placeholder="ej: 000123456" value={form.nTransferencia} onChange={e=>updateForm({nTransferencia:e.target.value})}/></div>
                  </div>
                  {form.fechaPago&&<div style={{marginTop:8,fontSize:12,color:"#86efac"}}>✓ Pago registrado el {form.fechaPago}</div>}
                </div>

                <div><label style={labelSt}>Estado</label>
                  <select style={S.input} value={form.estado} onChange={e=>updateForm({estado:e.target.value})}>
                    <option value="pendiente">⏳ Pendiente pago</option>
                    <option value="pagada">💳 Pagada (transferencia)</option>
                ...
                <div><label style={labelSt}>Observaciones</label><input style={S.input} value={form.obs} onChange={e=>updateForm({obs:e.target.value})}/></div>

                {/* Ingreso a stock bodega */}
                {!editId&&(
                  <div style={{gridColumn:"1/-1",display:"flex",alignItems:"center",gap:10,background:"rgba(61,122,82,0.08)",borderRadius:8,padding:"10px 14px",cursor:"pointer"}} onClick={()=>updateForm({ingresoStock:!form.ingresoStock})}>
                    <div style={{width:20,height:20,borderRadius:5,border:`2px solid #3d7a52`,background:form.ingresoStock?"#3d7a52":"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
                      {form.ingresoStock&&<span style={{color:"#fff",fontSize:13}}>✓</span>}
                    </div>
                    <div>
                      <div style={{fontSize:13,color:"#a0d8b0",fontWeight:600}}>📦 Ingresar al stock de Bodega</div>
                      <div style={{fontSize:11,color:"#5a8a6a"}}>Crea automáticamente una entrada en materiales de Bodega (Macrozona 39)</div>
                    </div>
                  </div>
                )}
              </div>
          ...
          {/* Lista */}
          {compras.filter(c=>{const mc=filtroCuenta==="todas"||c.cuenta===filtroCuenta;const mm=filtroMes==="todos"||(c.fecha||"").slice(0,7)===filtroMes;return mc&&mm;}).length===0&&!showForm?(
            <div style={{...S.card,padding:36,textAlign:"center",color:"#4a8a5a"}}><div style={{fontSize:32,marginBottom:8}}>🛒</div><div>Sin compras registradas</div></div>
          ):(
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              {compras.filter(c=>{const mc=filtroCuenta==="todas"||c.cuenta===filtroCuenta;const mm=filtroMes==="todos"||(c.fecha||"").slice(0,7)===filtroMes;return mc&&mm;}).map(c=>{
                const est=ESTADO_C[c.estado]||ESTADO_C.pendiente;
                const sel=seleccionadas.includes(c.id);
                const selectable=["pendiente","pagada"].includes(c.estado);
                return (
                  <div key={c.id} style={{...S.card,padding:14,borderLeft:`3px solid ${sel?"#a78bfa":est.color}40`,background:sel?"rgba(167,139,250,0.06)":"transparent"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"start",flexWrap:"wrap",gap:8}}>
                      <div style={{display:"flex",gap:10,alignItems:"start",flex:1,minWidth:0}}>
                        {selectable&&(
                          <div style={{width:18,height:18,borderRadius:4,border:`2px solid ${sel?"#a78bfa":"rgba(255,255,255,0.2)"}`,background:sel?"#a78bfa":"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,cursor:"pointer",marginTop:2}}
                            onClick={()=>setSeleccionadas(p=>sel?p.filter(x=>x!==c.id):[...p,c.id])}>
                            {sel&&<span style={{color:"#fff",fontSize:11}}>✓</span>}
                          </div>
                        )}
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap",marginBottom:4}}>
                            <span style={{fontFamily:"'Playfair Display',serif",fontSize:14,fontWeight:700}}>{c.descripcion}</span>
                            <span style={{...S.chip,background:est.bg,color:est.color,border:`1px solid ${est.color}40`,fontSize:10}}>{est.label}</span>
                            {c.cuenta&&<span style={{...S.chip,background:"rgba(59,130,246,0.1)",color:"#93c5fd",border:"1px solid rgba(59,130,246,0.2)",fontSize:10}}>🏷️ {c.cuenta}</span>}
                          </div>
                          <div style={{display:"flex",gap:10,flexWrap:"wrap",fontSize:11,color:"#7aaa80"}}>
                            <span>📅 {c.fecha}</span>
                            {c.proveedor&&<span>🏪 {c.proveedor}</span>}
                            {c.nDocumento&&<span>📄 {c.tipoDoc} {c.nDocumento}</span>}
                            {c.responsable&&<span>👤 {c.responsable}</span>}
                            {c.fechaPago&&<span style={{color:"#86efac"}}>💳 Pagado {c.fechaPago}{c.bancoPago&&` · ${c.bancoPago}`}</span>}
                            {c.nTransferencia&&<span style={{color:"#86efac"}}>🔢 Trans: {c.nTransferencia}</span>}
                          </div>
                          {c.obs&&<div style={{fontSize:11,color:"#5a8a6a",fontStyle:"italic",marginTop:2}}>{c.obs}</div>}
                        </div>
                      </div>
                      <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:5,flexShrink:0}}>
                        <span style={{fontFamily:"'Playfair Display',serif",fontSize:16,fontWeight:700,color:"#93c5fd"}}>${Number(c.totalBruto||c.totalNeto||0).toLocaleString("es-CL")}</span>
                        {c.totalNeto&&c.totalBruto&&Number(c.totalNeto)!==Number(c.totalBruto)&&<span style={{fontSize:10,color:"#5a8a6a"}}>Neto ${Number(c.totalNeto).toLocaleString("es-CL")}</span>}
                        <div style={{display:"flex",gap:5}}>
                          {c.estado==="pendiente"&&<button style={{...S.btn,fontSize:10,padding:"3px 8px",background:"rgba(59,130,246,0.15)",color:"#93c5fd",border:"1px solid rgba(59,130,246,0.3)"}} onClick={()=>marcarPagada(c.id)}>💳 Pagada</button>}
                          <button style={{...S.btn,fontSize:10,padding:"3px 8px",background:"rgba(61,122,82,0.15)",color:"#86efac",border:"1px solid rgba(61,122,82,0.3)"}} onClick={()=>{ingresarAStock(c);alert(`✅ "${c.descripcion}" ingresado al stock de Bodega`);}}>📦 Stock</button>
                          {esJefa&&<button className="btn-g" style={{...S.btn,fontSize:10,padding:"3px 8px"}} onClick={()=>editar(c)}>✏️</button>}
                          {esJefa&&<button className="btn-d" style={{...S.btn,fontSize:10,padding:"3px 8px"}} onClick={()=>eliminar(c.id)}>🗑</button>}
                        </div>
                      </div>
              ...
              {rendiciones.map(r=>{
                ...
                return (
                  <div key={r.id} style={{...S.card,padding:16,borderLeft:`3px solid ${r.reembolso?"#22c55e":"#a78bfa"}`}}>
                    ...
                    <div style={{borderTop:"1px solid rgba(255,255,255,0.07)",paddingTop:10,marginBottom:10}}>
                      {itemsRend.map(c=>(
                        <div key={c.id} style={{display:"flex",justifyContent:"space-between",fontSize:12,color:"#7aaa80",padding:"3px 0",borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
                          <span>{c.descripcion} {c.proveedor&&`· ${c.proveedor}`} {c.nDocumento&&`· ${c.tipoDoc} ${c.nDocumento}`}</span>
                          <span style={{fontWeight:600,color:"#93c5fd",flexShrink:0,marginLeft:8}}>${Number(c.totalBruto||c.totalNeto||0).toLocaleString("es-CL")}</span>
                        </div>
                      ))}
                    </div>
                    ...
                    {r.reembolso&&(
                      <div style={{background:"rgba(34,197,94,0.08)",border:"1px solid rgba(34,197,94,0.2)",borderRadius:8,padding:"10px 14px"}}>
                        ...
                        <div style={{display:"flex",gap:14,flexWrap:"wrap",fontSize:12,color:"#5aaa7a"}}>
                          ...
                          {r.bancoReembolso&&<span>🏦 {r.bancoReembolso}</span>}
                          {r.nTransReembolso&&<span>🔢 Trans: {r.nTransReembolso}</span>}
                          {r.obsReembolso&&<span>{r.obsReembolso}</span>}
                        </div>
                      </div>
                    )}
                    ...
                    {pendReemb&&esJefa&&(
                      showReembolsoForm===r.id?(
                        <div style={{background:"rgba(34,197,94,0.05)",border:"1px solid rgba(34,197,94,0.2)",borderRadius:8,padding:"12px 14px",marginTop:10}}>
                          ...
                          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10}}>
                            <div><label style={labelSt}>Fecha transferencia</label><input type="date" style={S.input} value={reembolsoForm.fecha} onChange={e=>setReembolsoForm(p=>({...p,fecha:e.target.value}))}/></div>
                            <div><label style={labelSt}>Monto reembolsado ($)</label><input type="number" style={S.input} placeholder={r.total} value={reembolsoForm.monto} onChange={e=>setReembolsoForm(p=>({...p,monto:e.target.value}))}/></div>
                            <div><label style={labelSt}>Banco origen (Estadio)</label><input style={S.input} placeholder="ej: Banco de Chile" value={reembolsoForm.banco} onChange={e=>setReembolsoForm(p=>({...p,banco:e.target.value}))}/></div>
                            <div><label style={labelSt}>N° Transferencia</label><input style={S.input} placeholder="ej: 000123456" value={reembolsoForm.nTransferencia} onChange={e=>setReembolsoForm(p=>({...p,nTransferencia:e.target.value}))}/></div>
                            <div style={{gridColumn:"1/-1"}}><label style={labelSt}>Observaciones</label><input style={S.input} value={reembolsoForm.obs} onChange={e=>setReembolsoForm(p=>({...p,obs:e.target.value}))}/></div>
                          </div>
                          <div style={{display:"flex",gap:8}}>
                            <button className="btn-p" style={{...S.btn,background:"#166534",color:"#86efac"}} onClick={()=>registrarReembolso(r.id)}>✓ Confirmar reembolso</button>
                            <button className="btn-g" style={S.btn} onClick={()=>setShowReembolsoForm(false)}>Cancelar</button>
                          </div>
                        </div>
                      ):(
      ...
      {subTab==="cuentas"&&(
        <div className="ein">
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:16,marginBottom:14,color:"#a0d8b0"}}>Cuentas / Ramas de imputación</div>
          {esJefa&&(
            <div style={{marginBottom:16}}>
              {!showCuentaForm?<button className="btn-p" style={S.btn} onClick={()=>setShowCuentaForm(true)}>➕ Nueva cuenta</button>:(
                <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                  <input style={{...S.input,flex:1,minWidth:180}} placeholder="Nombre de la cuenta..." value={nuevaCuenta} onChange={e=>setNuevaCuenta(e.target.value)} onKeyDown={e=>e.key==="Enter"&&agregarCuenta()}/>
                  <button className="btn-p" style={S.btn} onClick={agregarCuenta}>Agregar</button>
                  <button className="btn-g" style={S.btn} onClick={()=>{setShowCuentaForm(false);setNuevaCuenta("");}}>Cancelar</button>
                </div>
              )}
            </div>
          )}
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {cuentas.map(cu=>{
              const n=compras.filter(c=>c.cuenta===cu).length;
              const total=compras.filter(c=>c.cuenta===cu).reduce((a,c)=>a+Number(c.totalBruto||c.totalNeto||0),0);
              const esBase=["Rama Golf","Mantenimiento Jardines","Obras","Insumos Generales","Maquinaria y Equipos","Fitosanitarios","Semillas y Plantas","Uniformes y EPP"].includes(cu);
              return (
                <div key={cu} style={{...S.card,padding:"12px 16px",display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    <span>🏷️</span>
                    <span style={{fontSize:14,fontWeight:600}}>{cu}</span>
                    {esBase&&<span style={{fontSize:10,color:"#4a7a5a",background:"rgba(255,255,255,0.05)",padding:"1px 6px",borderRadius:6}}>base</span>}
                  </div>
                  <div style={{display:"flex",gap:12,alignItems:"center"}}>
                    <span style={{fontSize:12,color:"#6aaa7a"}}>{n} compra{n!==1?"s":""}</span>
                    {total>0&&<span style={{fontSize:13,fontWeight:700,color:"#93c5fd"}}>${total.toLocaleString("es-CL")}</span>}
                    {esJefa&&!esBase&&n===0&&<button className="btn-d" style={{...S.btn,fontSize:10,padding:"3px 8px"}} onClick={()=>set({cuentas:cuentas.filter(c=>c!==cu)})}>🗑</button>}
                  </div>
                </div>
              );
            })}
